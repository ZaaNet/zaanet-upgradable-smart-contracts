...
contract ZaaNetAdmin is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public lastEmergencyToggleTime = 0;
    ...
    event HostVoucherFeeTierUpdated(uint8 indexed tier, uint256 indexed newFee, address indexed changedBy);
    event PaymentAddressUpdated(address indexed oldAddress, address indexed newAddress, address indexed changedBy);
    ...
    constructor(
        ...
        require(_platformFeePercent <= MAX_PLATFORM_FEE, "Platform fee exceeds maximum");
    ...
    function setHostVoucherFeeHours(uint256 _fee) external onlyOwner notInEmergencyMode {
    ...
    function setHostVoucherFeeDays(uint256 _fee) external onlyOwner notInEmergencyMode {
        require(_fee <= MAX_HOST_VOUCHER_FEE, "Fee exceeds maximum");
    ...
    function setHostVoucherFeeMonths(uint256 _fee) external onlyOwner notInEmergencyMode {
        require(_fee <= MAX_HOST_VOUCHER_FEE, "Fee exceeds maximum");
    ...
    function setPaymentAddress(address _newPaymentAddress) external onlyOwner notInEmergencyMode {
        ...
        emit PaymentAddressUpdated(oldPaymentAddress, _newPaymentAddress, msg.sender);
    }
